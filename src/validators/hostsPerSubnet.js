 ...
 * @returns {Validation}
 */
export default function hostsPerSubnetIsNumber(hostsPerSubnet: number): Validation {
  return (_isNumber(hostsPerSubnet)) ? Validation.Success() : Validation.Failure([`'hostsPerSubnet' must be an integer.`]);
};

export default function hostsPerSubnetValidRange(hostsPerSubnet: number, totalHosts: number): Validation {
...
export function hostsPerSubnetValidSizeValidator(totalHosts: number): Validation {
  return (HOSTS_TO_PREFIX[totalHosts]) ? Validation.Success() : Validation.Failure([`'hostsPerSubnet' must be an integer that corresponds to a valid CIDR prefix.`]);
}
